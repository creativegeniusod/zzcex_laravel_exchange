      ...
      @if(isset($current_orders_user))
        ...
        @foreach($current_orders_user as $order)
          ...
          @endif
            ...
            <td><button onclick="javascript:cancelOrder({{{$order->id}}});" class="btn btn-xs btn-danger">{{trans('texts.cancel')}}</button></td>
          </tr>
        @endforeach
      @endif
    </tbody>
...
function cancelOrder(order_id){
        $.post('<?php echo action('OrderController@doCancel')?>', {isAjax: 1, order_id: order_id }, function(response){
              ...
              if(obj.status == 'success'){
				...
				$('#orderMsg').html('<div class="alert alert-danger text-center" role="alert">'+obj.message+'</div>');
				setTimeout(function() {
			      $('#orderMsg').fadeOut('slow');
				  location.reload()
				}, 1000);
