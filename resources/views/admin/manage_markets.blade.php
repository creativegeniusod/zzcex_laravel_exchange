...
<form class="form-horizontal" role="form" id="add_new_market" method="POST" action="{{{ url('/admin/add-market') }}}">
	 {{ csrf_field() }}
...
function deleteMarket(market_id){
	...
	if (r == true) {
		$.post('<?php echo action('AdminAuth\Admin_SettingController@deleteMarket')?>', {isAjax: 1, market_id: market_id }, function(response){
...
function disableMarket(market_id,status){
	$.post('<?php echo action('AdminAuth\Admin_SettingController@disableMarket')?>', {isAjax: 1, market_id: market_id, status:status }, function(response){
       	var obj = $.parseJSON(response);
	    console.log('obj: ',obj);
	    if(obj.status == 'success'){
            $('#messageModal .modal-body').html('<p style="color:#008B5D; font-weight:bold;text-align:center;">'+obj.message+'</p>');
