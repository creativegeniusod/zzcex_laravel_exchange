...
<form class="form-horizontal" role="form" id="add_new_wallet" method="POST" action="{{{ url('/admin/add-wallet') }}}" enctype="multipart/form-data">
	 {{ csrf_field() }}
...
function deleteWallet(wallet_id){
	...
	if (r == true) {
	   $.post('<?php echo action('AdminAuth\Admin_SettingController@deleteWallet')?>', {isAjax: 1, wallet_id: wallet_id }, function(response){
