...
<a href="javascript:void()" id="add_user_link">{{trans('admin_texts.add_user')}}</a>
	...
	@endif
<form class="form-horizontal" role="form" id="add_new_user" method="POST" action="{{{ url('/admin/add-user') }}}">
...
function deleteUser(user_id){
	...
	if (r == true) {
		$.post('<?php echo action('AdminAuth\Admin_SettingController@deleteUSer')?>', {isAjax: 1, user_id: user_id }, function(response){
...
function banUser(user_id,status){
	$.post('<?php echo action('AdminAuth\Admin_SettingController@banUSer')?>', {isAjax: 1, user_id: user_id, status:status }, function(response){
       	var obj = $.parseJSON(response);
	    console.log('obj: ',obj);
	    if(obj.status == 'success'){
            $('#messageModal .modal-body').html('<p style="color:#008B5D; font-weight:bold;text-align:center;">'+obj.message+'</p>');
