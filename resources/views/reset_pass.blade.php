      ...
      <div id="resetpassfrom" class="formuser">
        ...
        <form method="POST" action="{{{ (Confide::checkAction('UserController@do_reset_password'))    ?: URL::to('/user/reset') }}}" accept-charset="UTF-8" class="reset_pas">
          ...
          <div class="form-group text">
              <input class="form-control" type="password" name="password" id="password" value="{{{ Input::old('email') }}}" placeholder="{{{ Lang::get('confide::confide.password') }}}"/>
              <br>
              <input class="form-control" placeholder="{{{ Lang::get('confide::confide.password_confirmation') }}}" type="password" name="password_confirmation" id="password_confirmation">
          </div>
          <button tabindex="3" type="submit" class="btn btn-lg btn-primary">{{{ Lang::get('confide::confide.forgot.submit') }}}</button>
      </form>
      </div>
    </div>
</div>
