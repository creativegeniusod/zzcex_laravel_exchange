...
class UserController extends Controller {
    ...
    public function checkCaptcha()
    {
        ...
        }else  exit("You did not submit the captcha!");
    }
    ...
    public function doWithdraw_bakup(){
        ...
        if(Hash::check($password, Auth::user()->password)) {
            ...
            if($amount < $min_amount){
                return Redirect::to('user/withdraw/'.$wallet->id)
                            ->with( 'error', "Amount to withdraw must be equal to or great than ".$min_amount."." );
        ...
        if(Hash::check($password, Auth::user()->password)) {
            ...
            try{
                ...
                if($amount < $min_amount){
                    ...
                    $withdraw->save();
                    Log::info("\n"."Add Withdraw Transaction. Sent email to confirm: ",array('user_id' => $user->id, 'wallet_id' => $wallet->id, 'to_address'=>$address, 'amount'=>$amount, 'fee_amount'=>$fee,'receive_amount'=>$net_total,'created_at'=>date('Y-m-d H:i:s'),'updated_at'=>date('Y-m-d H:i:s'),'status'=>0,'transaction_id'=>'','confirmation_code'=>$confirmation_code));
                    if($withdraw->id){
                        //send mail confirm
                        ...
                        return Redirect::to('user/withdraw/'.$wallet->id)
                                    ->with( 'success', "A confirmation e-mail was sent to your e-mail. Please check your e-mail to confirm the withdrawal.");
                    }else{
                        return Redirect::to('user/withdraw/'.$wallet->id)
                                    ->with( 'error', "Can't insert the withdrawal to database." );
                    }
            ...
            if(isset($withdraw->to_address)){
                if($confirmation_code==$withdraw->confirmation_code && $withdraw->status==0){
                    ...
                    try{
                        ...
                        if($balance_amount>$withdraw->amount && $admin_balance>$withdraw->amount){
                            ...
                            if($txid && $balance->takeMoney($withdraw->amount,$wallet->id,$withdraw->user_id)){
                                ...
                                return Redirect::to('user/withdraw/'.$wallet->id)
                                ->with( 'error', "Sorry. We do not have enough funds in our hot wallet. Please withdraw later." );
                        }else{
                            return Redirect::to('user/withdraw/'.$wallet->id)
                            ->with( 'error', "Your balance is not enough." );
                        }
                    ...
                    return Redirect::to('user/profile/withdrawals')
                            ->with( 'error', "The confirmation code is not matching." );
                }
            }else{
                return Redirect::to('user/profile/withdrawals')->with( 'error', "Couldn't find this transaction's withdrawals.");
            }
        }else{
            return Redirect::to('user/profile/withdrawals')->with( 'error', "Couldn't find this transaction's withdrawals.");
        }
    ...
    public function cancelWithdraw(){
        ...
        if($withdraw->user_id == $user->id){//this condition use to avoid case a user cancel order of other user
            //delete order
            $withdraw->delete();
            echo json_encode( array('status'=>'success','message'=> "The withdrawal ".$withdraw_id." was cancled!" ) );
            exit;
        }else{
            echo json_encode(array('status'=>'error','message'=> "Sorry. You are not allowed to cancel this withdrawal!"));
            exit;
        }
    }
    ...
    public function referreredTradeKey(){
        ...
        if(isset($user_referred->username) && $user_referred->id!=$user->id){
            ...
            return Redirect::to('user/profile/dashboard')
                            ->with( 'error', 'Sorry. You cant referrer yourself!' );
        }else{
            return Redirect::to('user/profile/dashboard')
                            ->with( 'error', 'Sorry. Trade key does not exist!' );
        }
    }
        ...
        if(Hash::check($password, $user->password)) {
            ...
            if(!isset($user_receive->username)){
                return Redirect::to('user/transfer-coin/'.$wallet->id)
                                ->with( 'error', 'Sorry. That trade key does not exist!');
